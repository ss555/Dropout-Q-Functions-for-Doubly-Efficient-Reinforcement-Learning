...
from rlutils.vision import red_point, yellow_point, blue_line,mark_episode,crop_red_point
...
class camera_linear(gym.Env):
    ...
    def __init__(self, cap: cv2.VideoCapture, pi_conn, _max_episode_steps=768, act_scale=40, reward_mode='speed',
                 speed_buf_len=20, tau=0.05, img_crop=True):
        ...
        self.FORMAT = 'utf-8'
        self.act_scale = act_scale
        _, self.obs = self.cap.read()
        self.bx, self.by, _ = blue_line(self.obs)
        ...
        self.cx, self.cy = red_point(self.obs)
    ...
    def step(self, action):
        ...
        self.step_counter += 1
        # reward and done
        rew, flag = self.calculate_rew_speed_flag(self.obs)
        done = (True if self.step_counter >= self.N else False) or flag == 'done'
        return self.process(self.obs), rew, done, {}
    ...
    def calculate_rew_speed_flag(self, obs):
        ...
        self.last_pos = self.pos
        # rew = np.mean(self.speed_buf) # projection/norm #deprecated
        rew = -0.1  # timestep penality
...
class camera_base(gym.Env):
    ''' BASE ENV
    This is a fish linear movement camera environment
    1. tracks red points in the image and gives reward accordingly to the advancement along the blue line of the tank.
    '''
    def __init__(self, cap: cv2.VideoCapture, pi_conn, act_scale=40,img_crop=True,tau=0.05):
        ...
        self.coun=0#RESET-RPI
        if self.img_crop:
            print(f'using centered images camera-base')
        self.cx, self.cy = red_point(self.obs)
    ...
    def step(self, action):
        ...
        #WAIT tau seconds for action to take effect on the dynamics
        if timer()-self.last_timer<self.tau:
            try:
                time.sleep(timer()-self.last_timer)
            except:
        ...
        self.last_timer = timer()
        # reward and done
        _, flag = mark_episode(self.obs, self.cx, self.cy)
        return self.process(self.obs), -0.1, flag == 'done', {}
    ...
    def reset_raspi(self):
        '''
        reset the fish to ini position
        '''
        self.conn.sendall(str('RESET').encode(self.FORMAT))
        sData = self.conn.recv(124).decode(self.FORMAT)
        flag=''
...
class fish_exp_red_yellow(camera_base):
    '''
    concatenate latent space and action of servo and position cx,cy

    History should add action and repeat sn=4(s,a)
    '''
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        self.observation_space=spaces.Box(low=-2000, high=2000, shape=(4,), dtype=np.uint8)
    ...
    def step(self, action):
        ...
        _, self.obs = self.cap.read()
        self.calc_state()
        _, flag = mark_episode(self.obs, cx=self.cx, cy=self.cy, px=self.px, py=self.py)
        if flag=='done':
            rew=1
        else:
            rew=-0.1
        return self.state, rew, flag == 'done', {}

    def calc_state(self):
        self.cx, self.cy = red_point(self.obs)
        self.px, self.py = yellow_point(self.obs)
        self.state = np.array([self.cx, self.cy, self.px, self.py])
    def reset(self):
        self.reset_raspi()
        ret, self.obs = self.cap.read()
        self.calc_state()
...
class fish_exp_red_yellow_speed(fish_exp_red_yellow):
    '''
    concatenate latent space and action of servo and position cx,cy
    '''
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        self.len=20
        self.speed_buf = deque(np.zeros(self.len), maxlen=self.len)
        self.pos = 0
        self.last_pos = 0
        _, self.obs = self.cap.read()
        self.bx, self.by, _ = blue_line(self.obs)
    ...
    def step(self, action):
        ...
        self.speed_buf.append((self.pos-self.last_pos)/self.tau)
        self.last_pos = self.pos
        return self.state, np.mean(self.speed_buf), flag == 'done', {}
    ...
    def ini_state(self):
        ...
        self.calc_state()
        self.calc_pose()
        self.speed_buf = deque([],maxlen=self.len)
...
class fish_exp_vae_multi(camera_linear):
    ...
    def __init__(self,encoder,*args,**kwargs):
        ...
        self.state_len = self.encoder.fc_mu.bias.shape[0]+2#lat+x,y
    ...
    def encode(self,observation):
        with torch.no_grad():
            pr_img = torch.unsqueeze(self.transform(observation), 0)
            return self.encoder.forward(pr_img)[0].numpy()[0]
    def ini_state(self):#worth to init with cx,cy!?
        ret, self.obs = self.cap.read()
        self.ini_pos(self.obs)
        self.state = deque(np.zeros(self.state_len),maxlen=self.state_len)
        self.set_state(obs=self.obs)

    def set_state(self,obs,act=[0]):
        pr_img=self.process(obs)
        #DEb
        plt.imshow(pr_img)
        plt.show()
        self.state.extend([self.encode(pr_img), self.norm_img_pt(self.cx), self.norm_img_pt(self.cy)])
        # self.state.extend(self.encode(obs))
        # self.state.extend(np.array()
        # self.state.extend(np.array(np.array((self.cy-self.mean_pix)/self.std_cy).reshape(-1)))
    def norm_img_pt(self,p):
        return np.array((self.cx-self.mean_pix)/self.std_cx).reshape(-1)
    def step(self, action):
        #pwm and send/receive
        self.action = action
        ...
        self.step_counter += 1
        # reward and done
        rew, flag = self.calculate_rew_speed_flag_state(self.obs) #self.calc_pose()
        done = (True if self.step_counter >= self.N else False) or flag == 'done'
    ...
    def calculate_rew_speed_flag_state(self, obs):
        ...
        self.set_state(obs=obs, act=[self.action-1])
        return rew, flag


class fish_exp_vae_multi_speed(fish_exp_vae_multi):
    def __int__(self):
        self.speed_buf = deque(np.zeros(self.len), maxlen=self.len)
